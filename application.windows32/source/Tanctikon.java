...
public void draw() {
  synchronized (this){
    ...
    if(screen==pause_menu){
      ...
      rect(0,0,width,height);
      Box box = new Box(screen.sca(.2f), screen.sca(.6f), 11, height/40f);
      box.colorText=0;
      box.colorBack=-1;
...
public synchronized void addPlayer(int controlType){
  int pid=cPlayer++;
...
public synchronized void remPlayer(){
  if(cPlayer==0)return;
  int pid=--cPlayer;
...
class Replay{
  ...
  public synchronized void register(Move move){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Tanctikon" };
    if (passedArgs != null) {
